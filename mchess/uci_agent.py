    ...
    def __init__(self, appque):
        ...
        try:
            with open('uci_engines.json', 'r') as f:
                ...
                self.log.debug(self.engines)
        except Exception as e:
            self.log.error("Can't load uci_engines.json: {}".format(e))
            return

        self.log.debug('{} engines loaded.'.format(len(self.engines)))
        if len(self.engines['engines']) == 0:
            self.log.error("No engine defined! Check uci_engines.json.")
        ...
        self.log.debug('Loading engine {}.'.format(
